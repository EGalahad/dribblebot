...
class LeggedRobot(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.camera_orientation = torch.zeros(self.num_envs, 3, dtype=torch.float, device=self.device, requires_grad=False)
        self.camera_orientation[:, 0] = 1
        self.camera_orientation[:, 1] = 0
        self.camera_orientation[:, 2] = 0
    ...
    def _render_headless(self):
        if self.record_now and self.complete_video_frames is not None and len(self.complete_video_frames) == 0:
            ...
            if self.cfg.rewards.relative_use_head:
                FR_HIP_positions = self.head_offset
                FR_HIP_velocities = torch.cross(self.base_ang_vel, self.head_offset, dim=1) + quat_rotate_inverse(self.base_quat, self.base_lin_vel)
            else:
                ...
                FR_HIP_velocities = quat_rotate_inverse(self.base_quat, self.rigid_body_state.view(self.num_envs, -1, 13)[:,FR_shoulder_idx,7:10].view(self.num_envs,3))
